...
export interface AngleMetrics {
  roll: number; // left/right tilt, -90 to 90
  pitch: number; // back/front tilt, -90 to 90
  isLevel: boolean;
  direction:
    | "level"
    | "tilt_left"
...
export class PitchDetector {
  ...
  private onAngleChange: (metrics: AngleMetrics) => void;

  constructor(
    onAngleChange: (metrics: AngleMetrics) => void,
  ...
  private calculateAngleMetrics(accelerometerData: {
    x: number;
    y: number;
    z: number;
  }): AngleMetrics {
    ...
    const isLevel =
      Math.abs(roll) < this.config.rollTolerance &&
      Math.abs(Math.abs(pitch) - this.config.pitchVertical) <
        this.config.pitchTolerance;

    // Determine direction and severity
    let direction: AngleMetrics["direction"] = "level";
    let severity: AngleMetrics["severity"] = "good";
    ...
    if (!isLevel) {
      ...
      if (
        ...
        const pitchDelta = Math.abs(
          Math.abs(pitch) - this.config.pitchVertical
        );
        if (pitchDelta > 45) {
          severity = "major";
...
export const calculateAngleColor = (
  severity: AngleMetrics["severity"]
): string => {
  switch (severity) {
    case "good":
      return "#4CAF50";
...
export const getAngleMessage = (metrics: AngleMetrics): string => {
  if (metrics.isLevel) {
...
export const getAngleMessageTranslated = (
  ...
  const severity =
    metrics.severity === "major" ? ` (${translations.adjustSeverity})` : "";
  return `${directionMsg}${severity}`;
};
